/** Documentation hub — 10-section guide covering all features. */
'use client';
import { useState, useEffect } from 'react';
import { BookOpen, Server, Shield, Monitor, Zap, HardDrive, Wrench, Globe, Cpu, Settings, FileText, ChevronRight, Search } from 'lucide-react';
import PublicHeader from '@/components/layout/public-header';
import PublicFooter from '@/components/layout/public-footer';

const sections = [
  {
    id: 'getting-started', icon: BookOpen, title: 'Getting Started', subtitle: 'Installation, setup, and first login',
    content: [
      { heading: 'Prerequisites', body: 'Universal iDRAC Console requires Docker and Docker Compose. The platform runs as a set of containers:\n\n• **PostgreSQL 16** — Primary database for server inventory, users, audit logs\n• **Redis 7** — Session caching and rate limiting\n• **API Server** — NestJS backend with Redfish/Legacy adapter layer\n• **Web Frontend** — Next.js 14 with Dell iDRAC 9-inspired theme\n\nMinimum requirements: 2 CPU cores, 2GB RAM, 10GB disk space.' },
      { heading: 'Quick Start with Docker', body: '```bash\ngit clone https://github.com/sumit-kumawat/universal-idrac-console.git\ncd universal-idrac-console\ncp .env.example .env\n# Edit .env with your secrets before first run\ndocker compose up -d\n```\n\nThe application will be available at `http://localhost:3000`. Register a new account to get started.' },
      { heading: 'Environment Variables', body: '• `POSTGRES_URL` — PostgreSQL connection string\n• `REDIS_URL` — Redis connection string\n• `JWT_SECRET` — Secret for JWT access token signing (use a strong random value)\n• `REFRESH_SECRET` — Secret for refresh token signing (use a strong random value)\n• `MASTER_ENCRYPTION_KEY` — AES-256 key for credential encryption (64 hex chars)\n• `NEXT_PUBLIC_API_URL` — Backend API URL for the frontend\n\n⚠️ Always use strong, unique secrets in production. Never use default values.' },
      { heading: 'First Login', body: 'After deploying the application, navigate to the registration page to create your first account:\n\n1. Go to `http://localhost:3000/register`\n2. Enter your organization name, email, and a strong password\n3. Your account will be created as the organization owner with full access\n\nThe super admin role has cross-tenant visibility — they can see all servers, users, and audit logs across all organizations. Keep your credentials secure and enable session timeout for additional security.' },
    ],
  },
  {
